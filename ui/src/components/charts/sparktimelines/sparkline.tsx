...
import {TimeSeriesPlot, TimeSeriesFill} from "../timeseries/plot"
...
export function SparkLine(props: SparkTimeLineProps) {
    ...
    const xScale = getTimeScale(props.stepExtent, chartWidth)
        ...
        <div className={'sparkline-content'} style={{width: `${titleWidth * 2 + chartWidth}px`}}>
            ...
            <svg className={'sparkline'} height={25} width={chartWidth}>
                ...
                <g transform={`translate(${0}, 25)`}>
                    <TimeSeriesPlot series={s} xScale={xScale} yScale={yScale} color={'#7f8c8d'}
                                    currentX={props.selected >= 0 ? props.currentX : null}/>
                    <TimeSeriesFill series={s} xScale={xScale} yScale={yScale} color={'#7f8c8d'} colorIdx={9}/>
                </g>
            </svg>
            {value}
        </div>
