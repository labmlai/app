...
import {defaultSeriesToPlot, getExtent, toDate} from "../utils"
...
function SparkLines(props: SeriesProps) {
    ...
    let sparkLines = track.map((s, i) => {
        ...
        return <SparkLine key={s.name} name={s.name} series={s.series} selected={props.plotIdx[i]}
                          stepExtent={[toDate(stepExtent[0]), toDate(stepExtent[1])]} width={rowWidth} onClick={onClick}
...
export function getSparkTimeLines(track: SeriesModel[] | null, plotIdx: number[] | null,
                                  width: number, onSelect?: ((i: number) => void), currentX?: any| null) {
    if (track != null) {
        if (track.length === 0) {
            return null
        }
